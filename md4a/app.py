...
def create_app() -> FastAPI:
    ...
    allowed_hosts = {host.strip() for host in allowed.split(",") if host.strip()} if allowed else None
    ...
    async def fetch(url: str = Query(..., description="Absolute page URL")) -> Response:
        try:
            ...
            raise HTTPException(status_code=502, detail=f"origin returned HTTP {exc.response.status_code}") from exc
        except httpx.RequestError as exc:
            raise HTTPException(status_code=502, detail=f"origin request failed: {exc}") from exc
        if not result.available:
            raise HTTPException(
                status_code=406,
                detail=f"origin returned {result.content_type or 'no content type'}, not text/markdown",
            )
